...
public class Blind extends Activity {
	private int serviceid = Constant.BLINDID;
	private Button up;
	private Button down;
	private Button clock;
	private Button cclock;
...
//      IPText.setText("192.168.1.9");
        ...
        clock = (Button)this.findViewById(R.id.clock);
        cclock = (Button)this.findViewById(R.id.cclock);
        up.setEnabled(false);
        down.setEnabled(false);
        clock.setEnabled(false);
        cclock.setEnabled(false);
      
        up.setOnClickListener(new Button.OnClickListener(){

			@Override
			public void onClick(View v) {
				Choices choice = new Choices(Constant.UP);
				try {
        ...
        down.setOnClickListener(new Button.OnClickListener(){
			...
			public void onClick(View arg0) {
				Choices  choice = new Choices(Constant.DOWN);
				try {
					fromClient.writeObject(choice);
				} catch (IOException e) {
					e.printStackTrace();
        ...
        cclock.setOnClickListener(new Button.OnClickListener(){
	...
	public void setUpIbtnListeners() {
		ibtn_dropDown.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (pop == null) {
					if (adapter == null) {
						adapter = new PopupAdapter(Blind.this);
						listView = new ListView(Blind.this);
